...
import getWorkFormatter from './get-works.formatter'

describe('Format fetched works data to display', () => {
  it('should return formatted works data', async () => {
    const fetchedData = await workHttpActionsStub.getWorks()

    const formattedWorks = getWorkFormatter(fetchedData)
    const expectedData = [
      {
    ...
    expect(formattedWorks).toEqual(expectedData)
  })
})
